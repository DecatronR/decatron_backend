...
const verifyNIN = async (req, res) => {
    ...
    if (ninResponse.data.summary.nin_check.fieldMatches.firstname && ninResponse.data.summary.nin_check.fieldMatches.lastname) {
        // Get the base64 passport image
        const passportBase64 = ninResponse.data.nin.photo; // assuming this is the base64 string
        
        // Create a unique filename 
        const uniqueFileName = `${Date.now()}-${ninResponse.data.nin.nin}-passport.jpg`;
        const uploadPath = path.join(__dirname, '../uploads/passports', uniqueFileName);
        ...
        try {
          ...
          const base64Data = passportBase64.replace(/^data:image\/\w+;base64,/, '');
        ...
        return res.status(400).json({
          success: false,
          responseCode: 400,
          responseMessage: "Firstname and Lastname do not match with NIN credentials",
        });
      }
