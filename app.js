...
// Initializing Socket.IO and attaching it to the server
const io = new Server(server, {
  cors: {
    origin: "http://localhost:3000", //connection to frontend origin
    methods: ["GET", "POST"],
    allowedHeaders: ["Content-Type"],
    credentials: true,
  },
...
io.on("connection", (socket) => {
  ...
  // Example event for receiving and broadcasting agent location
  socket.on("agentLocation", (location) => {
    console.log("Agent location received:", location);
    io.emit("agentLocationUpdate", location); // Broadcast to all clients
  });

  socket.on("buyerLocation", (location) => {
    console.log("Buyer location received:", location);
    io.emit("buyerLocationUpdate", location); // Broadcast to all clients
  });
